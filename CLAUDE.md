...
프로젝트에 스타일 하드코딩 부채가 남아 있다.

일괄 수정은 하지 않고, 다른 작업으로 파일을 열 때 함께 정리한다.

### 적용 대상

- 인라인 style의 색상·폰트·크기 지정

- text-[13px], bg-[#3182F6] 같은 Tailwind 임의값

- 폰트명 직접 지정

### 적용 제외 (고치지 말 것)

- <col style={{width}}> — HTML 스펙상 className 불가

- lib/generate-pdf.server.tsx, components/ExpensePDF.tsx — 별도 렌더링 엔진

- lib/daily-report.ts — 이메일 클라이언트용, 앱 CSS와 분리

### 범위 제한

- 이번 작업에서 실제로 수정하는 파일만 대상. import된 파일까지

  따라가지 말 것

- 기존 토큰으로 1:1 치환 가능한 것만. 토큰에 없는 값이면

  임의로 만들지 말고 목록으로 보고만 할 것

- 한 파일에서 정리 대상이 30곳을 넘으면 진행하지 말고 먼저 물어볼 것

### 작업 방식

1. 요청받은 작업을 먼저 완료하고 커밋

2. 스타일 정리는 별도 커밋으로 분리

3. 응답 마지막에 "스타일 정리: N곳 (파일명)" 한 줄로 보고

4. 디버깅·장애 대응 중에는 이 규칙을 적용하지 않음

### 진행 관리 - 스타일 정리를 수행한 파일은 STYLE-MIGRATION.md에서 체크 표시하고 정리한 개수를 기록한다

■ 최근 변경 (2026-07-31)
- **작업 정산 UX 개선 + 지출 기능 진입 경로 숨김** — 임시저장 목록 선박명 표시·가운데 정렬, 사전기입 인라인 편집(페이지 이동 없이 수정), 임시저장 '삭제' 라벨 분리. 지출(비용) 관련 진입 경로를 모바일·PC 전체에서 숨김(회계 프로그램 이관 결정, 서버 로직·기존 데이터는 유지).
- **스타일 하드코딩 부채 관리 체계 신설** — '스타일 정리 원칙'(점진적 적용) + `STYLE-MIGRATION.md`(57개 파일·816곳 체크리스트) + 죽은 'Spoqa Han Sans Neo' 폰트 하드코딩 16개 파일 정리(Pretendard 캐스케이드로 통일).
- **`/wrap-up`이 코드 변경까지 함께 배포하도록 개편** — 어제(07-30) 실제 화면 코드가 전량 미커밋인 채 남아있던 걸 오늘 발견(`/wrap-up`이 journal.md·CLAUDE.md만 커밋하던 설계 때문). 코드 변경도 사용자 확인 후 별도 커밋으로 함께 푸시하도록 재구성, 어제·오늘분 전부 이번에 함께 배포.
...
■ 테스트 (2026-07-29 실측 — 전부 통과)
- 단위 9 files / 196 pass — `lib/**` 8 + `app/**` 1 (vitest.config.ts include 기준)
  - bulk-submit / cost-calc / input-sanitize / number-format / pin-rate-limit / server-auth /
    spec-display / work-settlement-rates / work-settlement/_shared
- 통합 29 files / 112 pass (헬퍼 4종 airtable-store·fetch-mock·fixtures·setup 제외):
  - 골든패스 6 — 입고 / 출고 / 이동 / 지출 / 가공 / 작업정산
  - 직접등록 4 — 입고 / 출고 / 이동 / 지출 (PC 관리자 직접 등록 경로)
  - 반려·복구 6 — 입고 반려·재승인 / 출고 반려 / 출고 반려 LOT 실패 / 이동 반려 /
    이동 반려 부분실패 보상 / 이동 필드 복사
  - 정합성·동시성 6 — LOT 일련번호 동시성 / 출고 벌크 race / 출고 멱등 /
    출고 비용 PATCH 실패 / Idempotency payload 불일치 / 원가 이월(cost-carryover)
  - 정책 3 — 출고 벌크 정책 / 일괄 승인 정책 / PDF 발행 정책
  - 보안·검증·기타 4 — security / validation / 지출 권한(100만원) / extra
- npm scripts: test / test:integration / test:all
- Airtable in-memory store + fetch 모킹 + 외부(Resend/Blob/PDF/next-cache) mock
- 편중: 백엔드 도메인 로직(원가·이동·정합성)에 몰려 있고 **UI는 전량 손 도그푸딩** —
  개발 원칙 '검증 사각지대 명시'의 근거
...
■ 중기 목표 (2026-07-29 현황 재점검 — 남은 건 2건)
- ✅ 매입처/매입자/선박명 입고 폼 — 완료. 선박 정보 마스터까지 신설(2026-05-28)
- ✅ PDF 한글 폰트 임베드 — 완료 (`scripts/generate-font-base64.mjs`)
- ✅ PWA — 완료. 모바일(작업자)·PC(관리자) 2종 설치 가능, 아이콘 전면 교체 + favicon.ico
     404 해소(2026-07-06). 잔여는 maskable 아이콘·manifest screenshots 정도(무해)
- ⏳ **기기별 고정 로그인 — 미진행.** 인접 작업으로 로그인 기기 판별을 창 폭→입력 장치(포인터)
     기준으로 교체함(2026-07-04, `LoginShell.tsx`). '기기 기억'과는 별개 과제
- ⏳ **팩스 자동 발송 — 미진행**
- 🟡 인라인 폼 에러 메시지화 — 부분 (toast 통일까지, 인라인 정교화는 추후)

■ 장기 목표 (2026-07-29 현황 재점검)
- ⏳ PostgreSQL DB 이전 — Phase 5-A(조건부). 포석: LOT 상태 7필드에 영어 매핑 description
     기입해둠(2026-05-15). 트리거는 레코드 한계·API 속도·복잡 조인
- 🟡 AI 데이터 분석 — **인접 진입.** 어황일보 번역 봇(2026-07-27)으로 Claude 비전 API가
     프로덕션에 들어감. 다만 성격은 *업무 자동화*이고 **데이터 분석 자체는 미착수**
- 🟡 바코드 스캔 출고 — QR 외부 공개 페이지 `/lot/{LOT번호}` 완료(2026-05-20). 바코드는 별개
- ✅ **부자재 재고 확장 — 완료.** 부자재·경비 마스터 신설(2026-06-29, 4섹션 아이스팩/박스/
     내피/진공팩 + 박스당 단가) + LOT 판매원가 계산기에 연동. 목표 달성으로 기록만 남김
- ⏳ 검색 결과 자동 트리거 (URL 복원 시 검색 자동 실행 옵션)
- ⏳ LOT 일련번호 100% 동시성 — 현재 낙관적 재시도로 ~99%. 검증~POST 사이 race window 잔여.
     대안: Airtable 자동번호 또는 Vercel KV 분산락
- ⏳ FIFO 평단가 시스템

■ 현장 발생 트랙 (계획 밖에서 자란 기능 — 2026-07-29 신설)

중기·장기 목표에도 로드맵 Phase 표에도 없었으나 **업무 요구로 만들어져 지금은 핵심이 된 것들.**
'앞으로 할 일'이 아니라 **'이미 만들어진 것'의 기록** — 목표 목록과 성격이 달라 따로 둔다.
상세는 `docs/ROADMAP.md` 「현장 발생 트랙」.

- **A. 원가·가공 모델** (2026-06-30 ~ 07-16) — 원가 4단 사다리(재고원가→가공원가→판매원가→매출원가) /
  가공 거래(2단계 WIP, 입고·출고·이동과 나란한 4번째 재고 사건=변환) / 작업 정산 등록(원물 매입
  사이즈별 기록→확정 시 입고관리+LOT 자동 생성) / 3단계 화주 배분(프로토타입)
- **B. 어황일보 번역 봇** (2026-07-27, 라이브) — 조업보고서 사진→일본어 바이어 안내문.
  Telegram 웹훅 + Claude 비전 API (`lib/eohwang/*`). 프로덕션 `seafood-erp.vercel.app`
- **C. 디자인 시스템** (2026-07-27 ~ 07-28) — DESIGN.md + 디자인 토큰(이름=tailwind / 값=표면별 CSS
  변수) + 공통 컴포넌트 5종 + 표 표준(말줄임 폐지·숫자 셀 통일). **확산 미완**
- **D. PC 셸·출력** (2026-06) — 크롬식 탭바·창 안 분할 / A4 인쇄(재고장·통계) / CSV / QR 공개 페이지
- **E. 개발 인프라** (2026-06 ~) — 시멘틱 버저닝 / '검증 사각지대 명시' 원칙 / 용어 위키

  ※ 패턴: 계획이 놓친 자리는 **원가·정산의 실제 업무 절차**(A)와 **바이어 커뮤니케이션**(B)이었다.
    다음 계획 시 이 두 축을 먼저 살필 것.
...
- 연결점: Phase 3 IA 5번 "원가·손익" → 하위 "외부 ERP export" (현재 미구현)
  ※ 2026-07-14 '재고 작업' 카테고리 신설로 IA가 6→7 카테고리가 되며 원가·손익은 4번→5번
