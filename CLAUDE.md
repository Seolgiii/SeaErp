...
■ 최근 변경 (2026-07-31)
- **LOT 생애주기 반려 이벤트 필터링** — 재고를 안 움직이는 반려 건을 서버(`master-lot-timeline.ts`)에서 제외(통장의 순수 노이즈). 잔여 계산은 필터 전 완료·반려 delta 0이라 불변. 감사 추적은 결재 수신함/추후 HR 감사 소관이지 생애주기 아님(사용자 확인, 니즈 없음).
- **생애주기 출고 배지 색상 회색→빨강** — 입고(파랑)·출고(빨강)로 재고 증감 방향 즉시 구분. 이동 입고(초록)·이동 출고(주황) 유지. 실데이터(LOT 0180 필터·0140 빨강) 재구성 검증, 픽셀 육안 대기.
...
▸ 데이터 테이블 / 목록 화면 규칙 (2026-07-01) — "회계 사고 제로"의 연장. 숫자 오독·행 혼동이 실무 사고로 직결. **적용 범위 = `/admin` PC 데이터 화면 한정**(모바일 작업자 PWA는 토스 스타일 유지 — 표면이 달라 충돌 아님).
...
- 컬럼 폭 — 내용량에 비례. 균등 분할 금지(짧은 컬럼 빈 공간·긴 컬럼 잘림). 남는 폭은 실제 넓이가 필요한 컬럼(LOT번호 등)에 몰아줌. 같은 데이터를 보여주는 표끼리는 컬럼 폭·순서 통일(table-fixed + 공유 colgroup).
   - 공유 모듈 `app/admin/master/_lot-table.tsx` 신설(2026-07-01) — 3 워크리스트(가공·이동·출고)가 프리미티브 공유: NumBox(표시숫자 상자)·NumInputHeader(입력칸 중앙 헤더)·SectionTitle + softField·numCellInput·cellField·labelClass. LotCols(콜그룹)는 페이지마다 컬럼이 달라(출고엔 보관처 없음) 공유 안 함 → 각 페이지 로컬(가공은 검색표↔작업표 공유용 LotCols 유지).
...
- 숫자 컬럼(표시용) — 값을 고정폭 상자(`inline-block min-w-[Nch] text-right tabular-nums`)에 넣어 자릿수를 세로로 맞추되, **상자 자체는 셀 왼쪽**에 둔다. 셀 전체를 우측정렬하지 않는다(앞 컬럼과 틈 벌어지고 뒤 컬럼과 붙어 비대칭). 헤더는 같은 폭 상자로 감싸 **우측 끝을 숫자와 맞춘다.** min-w는 그 컬럼 최대 자릿수+여유 기준(초과 값은 그 행만 상자가 자람 — 용인). min-width는 **인라인 스타일(`style={{ minWidth }}`) 권장** — Tailwind v3 임의 rem값(`min-w-[3.5rem]`)이 dev HMR에서 CSS로 안 뽑히던 사례 있음(px 임의값은 무방). 규격·미수처럼 숫자로 보여도 사이즈 라벨인 값은 텍스트 취급(좌측).
...
- 기존 화면 수정 시 — 최소 수정 원칙(위 「개발 원칙」) 그대로. 위 규칙에 안 맞는 기존 코드를 발견해도 작업 범위 밖이면 고치지 말고 "이런 불일치가 있다"고 보고만.
   - 적용 현황(2026-07-01): 3 워크리스트(가공·이동·출고) **신 스펙 적용 완료**(노션풍 톤 + 표시숫자 상자 + 입력 중앙헤더, 공유 모듈 사용). 검색 결과 표현은 가공만 표, 이동·출고는 리스트(기존 구조 유지 — 표 통일은 별도). 앱 전체 다른 표(재고 조회·집계·손익·매입 통계 등)는 아직 "셀 전체 우측" 미준수 → 발견 시 보고만(일괄 개조 아님).
