...
- 부자재·경비 마스터 신설(Airtable MCP 테이블, 4섹션 아이스팩·박스·내피·진공팩, 박스당 단가) + LOT 생애주기 '판매원가 계산기'(재고원가 + 선택 부자재). 재고 상태 원가 라벨 '판매원가'→'재고원가' 정리(매출 맥락 `출고시점 판매원가` 필드는 유지). 빈 env 404 fix(테이블 경로 `?? → ||`).
