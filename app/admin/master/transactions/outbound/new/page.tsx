...
export default function OutboundCreatePage() {
  ...
  const runSearch = async () => {
    ...
    setSearching(true);
    try {
      const res = await searchLotByKeyword(q);
      if (!res.success) {
      ...
      const mapped: SearchHit[] = (res.records ?? []).map((r) => ({
        lotRecordId: r.id,
        lotNumber: str(r.fields['LOT번호']),
        product: str(r.fields['품목명']),
        spec: str(r.fields['규격']),
        misu: str(r.fields['미수']),
        available: numOf(r.fields['재고수량']),
        storage: str(r.fields['보관처']),
      }));
      setHits(mapped);
  ...
  const addRow = (h: SearchHit) => {
    ...
    setRows((prev) => [...prev, { ...h, qty: '', seller: '', price: '' }]);
  };
  ...
  const submit = async () => {
    ...
    if (rows.length === 0) {
      toast('출고할 LOT을 추가하세요.', 'error');
      return;
    }
    const parsed = tryParseInboundDateInput(bizDate);
    if (!parsed) {
      setBizDate(getSeoulTodaySlash());
      toast('출고일 형식이 올바르지 않아 오늘 날짜로 설정했습니다.', 'info');
    }
    const iso = parsed?.iso ?? getSeoulTodayISO();

    // 행별 검증 — 수량(1..재고)·판매처·판매가 필수
    const invalid = rows.some((r) => {
      const q = fromGroupedIntegerInput(r.qty).value;
      const p = fromGroupedOptionalIntInput(r.price).value;
      return !(q >= 1 && q <= r.available) || !r.seller.trim() || !(p != null && p > 0);
    });
    if (invalid) {
      toast('각 행의 출고수량(1~재고)·판매처·판매가를 확인하세요.', 'error');
      return;
    }
      ...
      for (const r of rows) {
        const res = await createOutboundDirect({
          ...
          item: {
            ...
            quantity: fromGroupedIntegerInput(r.qty).value,
            date: iso,
            seller: r.seller.trim(),
            salePrice: fromGroupedOptionalIntInput(r.price).value ?? 0,
            spec: r.spec,
            misu: r.misu,
          },
          commit,
  ...
  if (authError) {
    return (
      <div className="p-6">
        <Link
          href="/admin/master/transactions/outbound"
