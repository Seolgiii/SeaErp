...
// ─────────────────────────────────────────────────────────────────────────────
// 이동 PC 일괄 등록 (관리자 전용) — 노션풍 데이터 화면 규칙 적용(가공 투입과 동일 룩)
//   공통 프리미티브는 ../../../_lot-table
//
// ① 이동 정보(설정) → ② LOT 검색(도구) → ③ 작업목록(결과물·앵커): 행마다 이동수량·이동후보관처
//    → 전체 등록+승인. 행별 createTransferDirect(생성→승인) 순회, 부분 실패해도 나머지 진행·
//    실패 행만 잔류. 이동후보관처는 보관처 마스터에서만 선택(인라인 생성 안 함).
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div className="max-w-5xl space-y-7 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm leading-relaxed text-gray-500">
          LOT을 검색해 작업목록에 담고, 행마다 이동수량·이동후보관처를 정한 뒤 한 번에 등록합니다.
          ‘바로 승인’이 켜져 있으면 즉시 새 LOT이 생성되고 원본이 차감됩니다.
        </p>
      </div>

      {/* 이동 정보 (설정) */}
      <section>
        <SectionTitle icon={Cog6ToothIcon}>이동 정보</SectionTitle>
