...
async function buildQrDataUrl(lotNumber: string): Promise<string | null> {
  try {
    const url = `${getBaseUrl()}/lot/${encodeURIComponent(lotNumber)}`;
    ...
    logError("[public lot page] QR 생성 실패:", e);
    return null;
  }
}
    ...
    <main className="min-h-screen bg-[#F2F4F6] pb-10">
      ...
      <div className="px-5 pt-4 space-y-3">
        ...
        <div className="bg-white rounded-[24px] px-5 py-2 shadow-[0_8px_24px_rgba(149,157,165,0.08)]">
          ...
          />
          <Row label="원산지" value={detail.origin} />
          <Row
            label="입고일"
        ...
        <Link
          href={`/inventory/lot/${encodeURIComponent(detail.lotNumber)}`}
          className="block w-full py-3 rounded-2xl bg-white text-gray-700 text-center text-[13px] font-bold border border-gray-200 active:bg-gray-50 transition-all"
        >
          내부 상세 보기 (로그인 필요)
        </Link>
      </div>
...
export default async function PublicLotPage({
  ...
  return <PublicDetailView detail={detail} qrDataUrl={qrDataUrl} />;
}
