 ...
 *  - 외부창고 / 가공공장 / 기타  → 외부 시설 = 외부가 자체 발행 (false)
 ...
 */
export type StorageKind = "자사창고" | "외부창고" | "가공공장" | "기타";
 ...
 *   - 외부창고/가공공장/기타에서 자사창고로 들어올 때 → 출고증은 외부가 자체 발행
 *
 * 미분류(null) 처리:
 *   - 보관처 마스터에 `구분` 분류가 아직 안 된 record는 default true (현 동작 유지).
 *   - 운영 분류 완료 후 자연 활성화되어 외부 시설은 발행 X로 전환됨.
 */
