...
스타일 하드코딩 정리의 **규칙과 진행 상황을 함께 담은 정본**이다(2026-08-04에 규칙을
CLAUDE.md에서 이곳으로 옮겼다). 일괄 치환 대상 목록이 아니라, **다른 작업으로 파일을 열 때
함께 정리**하기 위한 체크리스트다.

## 정리 규칙

프로젝트에 스타일 하드코딩 부채가 남아 있다. 일괄 수정은 하지 않고, 다른 작업으로 파일을
열 때 함께 정리한다.

**적용 대상**
- 인라인 `style`의 색상·폰트·크기 지정
...
**범위 제한**
- 이번 작업에서 실제로 수정하는 파일만 대상. import된 파일까지 따라가지 말 것
- 기존 토큰으로 1:1 치환 가능한 것만. 토큰에 없는 값이면 임의로 만들지 말고 목록으로 보고만 할 것
...
- 어떤 이유로든 아래 목록의 파일을 열어 수정하게 되면, 위 적용 범위·제외 규칙에 맞는 곳만 함께 정리한다.
- 정리를 마치면 체크(`[x]`)하고, 괄호 안 개수를 실제 정리한 개수로 갱신한다(전량 정리 못 했으면 잔여 개수를 남긴다).
- **⚠ 표시 파일은 정리 대상이 30곳을 넘는다.** 위 범위 제한대로 임의로 진행하지 말고 먼저 사용자에게 확인한다.
...
**제외 대상**(위 「정리 규칙」의 적용 제외, 목록에 없음 — 애초에 이 검색 패턴에 해당 사항 없음):
